...
sub options {
    ...
    return (
        ...
        [ 'tag|t=s',      'the tag to add (optional)', { default => '' }, ],
        [ 'message|m=s@', 'change message to add',     { default => [] }, ],
        [   'dir|d=s@',
...
sub run {
    ...
    if ($svk->dist_current_version_is_tagged) {
        ...
        Dist::Joseki::Version->new->set_version($version,
            @{ $self->opt('dir') || [] });
    ...
    $changes->newest_release->tags_push($self->opt('tag'))
      if $self->opt_has_value('tag');
